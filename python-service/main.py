...
from db import (
    create_spec, get_spec, update_spec_status,
    insert_division, delete_divisions, get_divisions,
    insert_tiles_batch, delete_tiles, get_tiles_by_division, get_tiles_by_sections,
    insert_analysis
)
...
async def parse_spec_endpoint(spec_id: str):
    ...
    try:
        ...
        for div in result['divisions']:
            ...
            print(f"[PARSE]   Division {div['division_code']}: pages {div['start_page']}-{div['end_page']}")
...
async def analyze_spec_endpoint(spec_id: str, request: AnalyzeRequest):
    ...
    try:
        ...
        if not division_tiles:
            raise HTTPException(
                status_code=404,
                detail=f"No tiles found for Division {division}"
            )
        ...
        if request.include_contract_terms:
            ...
            if all_contract_tiles:
                div01_text = stitch_tiles(sorted(all_contract_tiles, key=lambda x: x.get('tile_index', 0)))
