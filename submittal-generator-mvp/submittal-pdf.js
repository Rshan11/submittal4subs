  ...
  for (let i = 0; i < items.length; i++) {
    ...
    for (const file of files) {
      try {
        ...
        const { data: fileData, error } = await supabase.storage
          ...
          .download(file.r2_key);

        if (error) {
          console.warn(`[PDF] Could not download file ${file.file_name}:`, error);
          continue;
        }
        ...
        console.log(`[PDF] Added ${copiedPages.length} pages from ${file.file_name}`);
      } catch (err) {
    ...
    if (files.length === 0) {
      ...
      placeholderPage.drawText('No documents attached', {
        x: (pageWidth - helvetica.widthOfTextAtSize('No documents attached', 14)) / 2,
        y: pageHeight / 2,
        size: 14,
        font: helvetica,
        color: mutedColor
...
export async function generateCoverSheetPreview(options) {
  ...
  page.drawText('SUBMITTAL PACKAGE', {
    x: (pageWidth - helveticaBold.widthOfTextAtSize('SUBMITTAL PACKAGE', 28)) / 2,
    y: pageHeight - 200,
    size: 28,
    font: helveticaBold,
    color: primaryColor
  ...
  page.drawText(options.projectName || 'Project Name', {
    x: (pageWidth - helveticaBold.widthOfTextAtSize(options.projectName || 'Project Name', 20)) / 2,
    y: pageHeight - 250,
    size: 20,
    font: helveticaBold,
    color: textColor
