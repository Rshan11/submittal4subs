...
export async function createSubmittalPackage(userId, jobId, jobName, items = []) {
  try {
    // Create the package
...
export async function uploadSubmittalFile(itemId, file) {
  try {
    ...
    const r2Key = `submittals/${itemId}/${timestamp}_${safeName}`;

    // Upload to R2 via your existing upload endpoint or Supabase storage
    // For now, using Supabase storage as fallback
...
export function parseSubmittalsFromAnalysis(analysisResult) {
  ...
  const sectionPattern = /Section\s+(\d{2}\s*\d{2}\s*\d{2})\s*[-–]\s*([^\n]+)/gi;
  let match;
  ...
  while ((match = productPattern.exec(text)) !== null) {
    ...
    const mfrMatch = details.match(/(?:by\s+|from\s+|mfr:\s*)([A-Z][a-zA-Z\s&]+?)(?:\s*[-,;]|$)/i);
  ...
  const quotePattern = /Quote\s+These\s+Items[:\s]*\n([\s\S]*?)(?=\n#|\n---|\n\*\*|$)/i;
  const quoteMatch = text.match(quotePattern);
  if (quoteMatch) {
    const items = quoteMatch[1].split('\n').filter(line => line.trim().startsWith('-') || line.trim().startsWith('•'));
    items.forEach(item => {
...
export function renderSubmittalGenerator(container, pkg, callbacks = {}) {
  const { onAddItem, onUpdateItem, onDeleteItem, onUploadFile, onDeleteFile, onCombine, onBack } = callbacks;
    ...
    <div class="submittal-generator">
      ...
      <div class="submittal-cards" id="submittalCards">
        ${pkg.items && pkg.items.length > 0 
          ? pkg.items.map((item, index) => renderSubmittalCard(item, index)).join('')
  ...
  // Event listeners
  container.querySelector('#backToResults')?.addEventListener('click', () => onBack?.());
  container.querySelector('#addItemBtn')?.addEventListener('click', () => onAddItem?.());
  container.querySelector('#combineBtn')?.addEventListener('click', () => onCombine?.());
